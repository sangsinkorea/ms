...
import { ProductInfo, PageLength, DetailImageSegment } from '../types/modusangse';
...
const DetailPlanner: React.FC<Props> = ({ apiKey, baseUrl }) => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // Auto analyze on upload if context is empty? Or just let user click?
            // Original code didn't auto-analyze in handleFileChange, it just set images.
            // Wait, previous attempt code showed it DID auto analyze in handleFileChange. 
            // The CURRENT file content (read in Step 360) lines 92-113 DOES NOT have auto analyze.
            // But the user logic might expect manual click "AI Example Analysis" button.
            // I will stick to the current logic which is just upload, but IF I want to be proactive...
            // Actually, keep it simple. Just update the Pros and the function that IS called.
    ...
    const handleAnalyzeImages = async () => {
        ...
        // Debugging feedback
        console.log("Starting analysis...", { hasImages, hasText, baseUrl });

        setAnalyzing(true);
        setError(null);
        try {
            const analysis = await analyzeProductFromImages(productInfo.referenceImages, productInfo.analysisContext, apiKey, baseUrl);
            console.log("Analysis result:", analysis);

            if (!analysis || Object.keys(analysis).length === 0) {
                throw new Error("AI가 응답하지 않았거나 결과를 추출하지 못했습니다.");
            }
            ...
            alert(`오류가 발생했습니다: ${e.message || "서버 통신 실패"}`);
        } finally {
            setAnalyzing(false);
        }
    };
    ...
    const generatePlan = async () => {
        ...
        try {
            setStep('PLANNING');
            const generatedPlan = await planDetailPage(productInfo, apiKey, baseUrl);
    ...
    const processSingleSegment = async (id: string) => {
        ...
        try {
            const imageUrl = await generateSectionImage(target, productInfo.referenceImages, apiKey, baseUrl);
            setPlan(prev => prev.map(p =>
